...
export const infiniteZoomingLevels = {
  ...
  async mounted () {
      ...
      let box = document.querySelector('#f2-1')
...
export const looping = {
  ...
  async mounted () {
    let previousHexagon = document.querySelector('.is-current-view > div > svg > g > #hexagon')
    let actualHexagon = document.querySelector('.is-new-current-view > div > svg > g > #hexagon')
    var color = getComputedStyle(previousHexagon).getPropertyValue(`stroke`)
    setTimeout(() => {
